...
import { Dropdown, Button, Badge, List, message, Spin, Space } // THÊM Spin, Space
    from 'antd';
...
    // Lấy số lượng chưa đọc & refetch
    const { data: unreadCount = 0, isFetching: isFetchingCount, refetch: refetchCount } = useGetUnreadCountQuery(userId || '', {
        skip: !userId,
...
    // --- Xử lý sự kiện Thông báo mới từ WebSocket (GIỮ NGUYÊN) ---
...
    // 🚀 Kết nối WebSocket
    useNotificationSocket({ onNewNotification: handleNewNotification, connectTrigger: !!userId });

    // --- XỬ LÝ ĐÁNH DẤU TẤT CẢ ĐÃ ĐỌC (Hàm mới) ---
...
        open={open}
        popupRender={menu => (
            <div style={{ width: 350, padding: 8, backgroundColor: '#fff', borderRadius: 8, boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' }}>
                <div style={{ padding: '0 8px 8px', fontWeight: 'bold' }}>
                    Thông báo ({isFetchingCount ? '...' : unreadCount} chưa đọc)
                </div>
                {isFetchingList ? 
                    <div style={{ padding: '10px', textAlign: 'center' }}><Spin /> Đang tải...</div> : 
                    (totalCount > 0 ? (
                        <List 
                            dataSource={items} 
                            renderItem={(item) => (
...
                            Đọc tất cả
                        </Button>
                        <Button type="link" onClick={() => console.log('Go to notification page')}>Xem tất cả</Button>
                    </Space>
                </div>
            </div>
        )}
