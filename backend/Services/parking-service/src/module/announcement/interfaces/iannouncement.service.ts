import type { AnnouncementResponseDto,CreateAnnouncementDto } from '../dto/announcement.dto';
import type { AnnouncementDocument } from '../schemas/announcement.schema';

export interface IAnnouncementService {
  findAll(): unknown;
  // Admin tạo thông báo
  create(dto: CreateAnnouncementDto, createdBy: string): Promise<AnnouncementResponseDto>;
  
  // Lấy danh sách Announcement đang chờ xử lý (dùng cho Cron Job)
  getPendingScheduledAnnouncements(currentTime: Date): Promise<AnnouncementDocument[]>;
