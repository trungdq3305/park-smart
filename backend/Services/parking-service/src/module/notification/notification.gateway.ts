import { UseGuards, ExecutionContext } from '@nestjs/common' // Giữ lại ExecutionContext
import {
  OnGatewayConnection,
  SubscribeMessage,
  WebSocketGateway,
...
  ): void {
    ...
    console.log(`[WS IDENTITY] Receiving payload. ID: ${userId}, Role: ${userRole}`);
    // =======================================================

    // Nếu decorator WsAuthUser gặp lỗi, các giá trị này có thể là undefined
    if (!userId || !userRole) {
        console.error(`[WS CRASH PREVENTED] Payload thiếu ID hoặc Role. ID: ${userId}, Role: ${userRole}`);
        client.emit(NotificationSocketEvents.AUTH_ERROR, 'Xác thực không đầy đủ (Thiếu ID/Role).');
    ...
    const roleRoom = String(userRole);
...
    console.log(`User ${userId} (${userRole}) joined rooms: ${userRoom}, ${roleRoom}`)
