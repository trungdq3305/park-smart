...
import {
  ...
  IsString,
  ValidateNested,
} from 'class-validator' // Thêm IsArray
...
export class ChatRequestDto {
  @ApiProperty({
    example: 'Tôi bị trừ điểm Uy Tín sai, tôi phải làm gì?',
    description: 'Câu hỏi mới nhất của người dùng.',
  })
  ...
  @ApiProperty({
    ...
    required: false,
    description:
      'Lịch sử chat dùng để duy trì ngữ cảnh. Gửi [] nếu là tin nhắn đầu tiên.',
  })
  @IsOptional()
...
export class ChatResponseData {
  @ApiProperty({
    example: 'Vui lòng tạo Báo cáo Sự cố (Report) trong ứng dụng...',
  })
  answer: string
}
  ...
  async chat(
    ...
    if (!chatRequestDto.newMessage || chatRequestDto.newMessage.trim() === '') {
      throw new BadRequestException(
        'Vui lòng nhập câu hỏi để trò chuyện với Trợ lý Hướng dẫn.',
      )
    }
