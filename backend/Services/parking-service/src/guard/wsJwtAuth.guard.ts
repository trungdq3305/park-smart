...
    // 1. Cố gắng đọc token từ query string (Cách truyền từ frontend hiện tại)
    let token = client.handshake.query.token as string | undefined
    
    // 2. Nếu không có trong query, Cố gắng đọc token từ Cookie Header (cho HttpOnly)
    if (!token && client.handshake.headers.cookie) {
        const parsedCookies = cookie.parse(client.handshake.headers.cookie)
        // Thay 'userToken' bằng tên cookie chứa JWT của bạn
...
    }
    ...
    console.log(`[WS Guard] Attempting to verify token: ${token.substring(0, 30)}...`);
    try {
...
        console.error('[WS Guard] Secret used (length):', secret ? secret.length : 'MISSING');
