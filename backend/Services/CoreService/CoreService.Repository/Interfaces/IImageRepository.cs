...
namespace CoreService.Repository.Interfaces
{
    public interface IImageRepository
    {
        ...
        Task<List<Image>> GetByOwnerAsync(OwnerType ownerType, string ownerId, int skip = 0, int limit = 50);

        Task<bool> HardDeleteAsync(string id);                           // xóa hẳn khỏi DB
        Task<long> HardDeleteByOwnerAsync(OwnerType ownerType, string ownerId);
